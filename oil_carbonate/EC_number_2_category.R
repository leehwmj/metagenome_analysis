...
#루프 만들어서 EC 골라내기
## EC number list 만들기
EC2_list <- paste("EC:2.", c(1:10), sep = "")
...
for(i in 1:length(EC2_list)){
  print (EC2_list[i])
  if(i == 1){
    a <- oil_KO_name |> filter(grepl(EC2_list[i], Name)) |> inner_join(oil_KO_tpm) |> select(ECHO_Blank, ECHO_C, ECHO_F) |> as.matrix() |> colSums() |> as.matrix()
  }else{
    a <- cbind(a, oil_KO_name |> filter(grepl(EC2_list[i], Name)) |> inner_join(oil_KO_tpm) |> select(ECHO_Blank, ECHO_C, ECHO_F) |> as.matrix() |> colSums() |> as.matrix())
  }
  colnames(a)[i] <- EC2_list[i]
}

EC2_all_sum <- t(a)
EC2_all_sum <- cbind(EC2_all_sum, EC2_list)[,c(4,1,2,3)]
rownames(EC2_all_sum)<-1:length(EC2_list)
colnames(EC2_all_sum)[1] <- "Name"

library(reshape2)
EC2_all_sum <- as.data.frame(EC2_all_sum)
EC2_melt <- melt(EC2_all_sum, id="Name")
str(EC2_melt)
EC2_melt[,3] <- as.double(EC2_melt[,3])
EC2_melt$variable <- factor(EC2_melt$variable, levels = c("ECHO_F","ECHO_C","ECHO_Blank"))
str(EC2_melt)

#
library(ggplot2)
library(plyr)
ggplot(EC2_melt, aes(x=Name, y=value, fill=variable))+
  geom_bar(stat = "identity",  colour="black")+
  scale_x_discrete(limits=rev(EC2_all_sum[,1]))+
  coord_flip() +
