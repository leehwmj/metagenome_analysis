...
for(i in 1:length(EC7_list)){
  print (EC7_list[i])
  if(i == 1){
    a <- oil_KO_name |> filter(grepl(EC7_list[i], Name)) |> inner_join(oil_KO_tpm) |> select(ECHO_Blank, ECHO_C, ECHO_F) |> as.matrix() |> colSums() |> as.matrix()
  }else{
    a <- cbind(a, oil_KO_name |> filter(grepl(EC7_list[i], Name)) |> inner_join(oil_KO_tpm) |> select(ECHO_Blank, ECHO_C, ECHO_F) |> as.matrix() |> colSums() |> as.matrix())
  }
  colnames(a)[i] <- EC7_list[i]
}

EC7_all_sum <- t(a)
EC7_all_sum <- cbind(EC7_all_sum, EC7_list)[,c(4,1,2,3)]
rownames(EC7_all_sum)<-1:length(EC7_list)
colnames(EC7_all_sum)[1] <- "Name"

library(reshape2)
EC7_all_sum <- as.data.frame(EC7_all_sum)
EC7_melt <- melt(EC7_all_sum, id="Name")
str(EC7_melt)
EC7_melt[,3] <- as.double(EC7_melt[,3])
EC7_melt$variable <- factor(EC7_melt$variable, levels = c("ECHO_F","ECHO_C","ECHO_Blank"))
str(EC7_melt)

#
library(ggplot2)
library(plyr)
ggplot(EC7_melt, aes(x=Name, y=value, fill=variable))+
  geom_bar(stat = "identity",  colour="black")+
  scale_x_discrete(limits=rev(EC7_all_sum[,1]))+
  coord_flip() +
